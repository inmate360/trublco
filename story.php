...
if(isset($_SESSION['user_id'])) {
    ...
    $user = $stmt->fetch();
    $is_user_admin = ($user && $user['is_admin'] == 1);
    $_SESSION['is_admin'] = isset($user['is_admin']) ? $user['is_admin'] : 0;
    if($debug_mode) $debug_messages[] = "Admin: " . ($is_user_admin ? 'YES' : 'NO');
}

if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['story_id']) && $is_user_admin) {
    if($debug_mode) $debug_messages[] = "POST: " . $_POST['action'] . " #" . $_POST['story_id'];
    try {
        $check = $db->query("SHOW COLUMNS FROM stories LIKE 'is_locked'");
        if($check->rowCount() == 0) {
            if($debug_mode) $debug_messages[] = "ERROR: is_locked column missing";
        } else {
...
<div style="position:fixed;top:10px;right:10px;z-index:9999;background:#000;border:2px solid #0f0;padding:15px;color:#0f0;font-family:monospace;font-size:12px;max-width:400px;">
<strong style="color:#ff0;">DEBUG</strong>
<button onclick="this.parentElement.remove()" style="float:right;background:#f00;color:#fff;border:none;padding:2px 8px;cursor:pointer;">X</button><br>
<?php foreach($debug_messages as $m): ?>
<div style="margin:3px 0;"><?php echo htmlspecialchars($m); ?></div>
<?php endforeach; ?>
</div>
<?php endif; ?>

<?php if(isset($_SESSION['user_id'])): ?>
<div style="position:fixed;bottom:10px;right:10px;background:<?php echo $is_user_admin?'#0f0':'#f00';?>;color:#000;padding:8px 12px;font-weight:bold;font-size:11px;">
<?php echo $is_user_admin?'ADMIN':'NOT ADMIN';?>
</div>
<?php endif; ?>
<style>
            ...
            <div class="flex items-center justify-between">
              ...
              <?php if($is_user_admin): ?>
                ...
                <form method="POST" class="inline" onsubmit="return confirm('<?php echo $story['is_locked'] ? 'Unlock' : 'Lock'; ?> this story?');">
                  ...
                  <?php if($story['is_locked']): ?>
                    <button type="submit" 
                            class="inline-flex items-center gap-2 px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 transition font-semibold text-sm">
                      <i class="bi bi-unlock-fill"></i>
                      Unlock
                    </button>
                  <?php else: ?>
                    <button type="submit" 
                            class="inline-flex items-center gap-2 px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition font-semibold text-sm">
